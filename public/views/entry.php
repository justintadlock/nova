	...
	<?php if ( 'aside' !== $format ) : ?>
		...
		<header class="entry__header max-w-2xl mx-8 sm:mx-auto">
			...
			<h2 class="entry__title m-0 mt-3">
				<a class="text-gray-800 no-underline hover:underline border-0" href="<?= e( $entry->uri() ) ?>"><?= widow( e( $entry->title() ) ) ?></a>
			</h2>
	...
	<?php else : ?>
		<div class="entry__summary o-content-width mt-4">
			<?= $entry->excerpt(
				50,
				sprintf(
					' &hellip; <a class="entry__more-link" href="%s">Continue reading</a>',
					e( $entry->uri() )
				)
			) ?>
		</div>
